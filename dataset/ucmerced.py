...
class UCMerced_Noise_Train(Dataset):
    def __init__(self,
                 config,
        ...
        for i in noise_im_ids:
            cate = int(i.strip().split()[1])
            img_path = os.path.join(
                self._base_dir, "Images", re.findall(r'[A-Za-z\']+', i)[0], str(i.strip().split()[0]))
            if self.obj[cate] != re.findall(r'[A-Za-z\']+', i)[0]:
                count += 1
            assert cate in range(0, 21)
        ...
        if self.train_states=='Mix':
            ...
            self.clean_images.clear()
            self.clean_targets.clear()
        # if self.train_states == 'Double':
        #     indices = np.random.permutation(len(self.clean_images))
        #     clean_indices_1,clean_indices_2=list(np.array_split(indices,2))
        #     self.clean_data_1=[self.clean_images[i] for i in clean_indices_1]
        #     self.clean_targets_1=[self.clean_targets[i] for i in clean_indices_1]
        #     self.clean_data_2=[self.clean_images[i] for i in clean_indices_2]
        #     self.clean_targets_2=[self.clean_targets[i] for i in clean_indices_2]
        #     self.noise_data_new=self.noise_images+self.clean_data_1
        #     self.noise_targets_new=self.noise_targets+self.clean_targets_1
        #     self.clean_data_new=self.clean_data_2
        #     self.clean_targets_new=self.clean_targets_2
        #     self.noise_images,self.noise_targets=self.noise_data_new,self.noise_targets_new
        #     self.clean_images,self.clean_targets=self.clean_data_new,self.clean_targets_new
...
if __name__ == '__main__':
    import argparse
    parser=argparse.ArgumentParser(description='PyTorch UCMerced Training')
    ...
    dst=UCMerced_Noise_Train(config=args)
