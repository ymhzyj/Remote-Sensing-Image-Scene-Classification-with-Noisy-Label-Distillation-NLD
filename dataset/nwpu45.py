...
class NWPU45_Noise_Train(Dataset):
    def __init__(self,
                 config,
        ...
        for i in noise_im_ids:
            cate = int(i.strip().split()[1])
            _realpath=re.findall(r'[A-Za-z\']+', i)
            _final=_realpath.index('jpg')
            realpath=""
            for j in range(0,_final):
                realpath+=_realpath[j]+'_'
            realpath=realpath.strip('_')
            img_path = os.path.join(
                self._base_dir, "Images", realpath, str(i.strip().split()[0]))

            if self.obj[cate] != realpath:
                count += 1
            assert cate in range(0, 45)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='PyTorch NWPU45 Training')
    ...
    dst = NWPU45_Noise_Train(config=args)
    val_dataloader = DataLoader(dst, batch_size=1)
    for i, (clean,noise) in enumerate(val_dataloader):
